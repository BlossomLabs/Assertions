...
- **`Expressions`** (unreleased) adds typed expression graphs and resolve-once call construction. A raw `InputParam` is a tree that cannot name a subterm, so a repeated expression is duplicated in calldata and resolved again each time; a graph references earlier nodes, evaluates each once per evaluation, binds whole canonical ABI values, branches lazily with a `Select` judged like the core's `cond`, and evaluates guarded attempts. The SDK does not compile against it yet; `Collections.Callback.expression` is its one in-tree consumer. See [Expressions](/docs/operators/expressions).
