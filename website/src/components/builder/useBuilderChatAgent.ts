...
Use @sender for the account sending the surrounding block's calls (the Safe, Governor or DAO executor when wrapped); @me is the connected wallet and @tx.from! is the transaction origin. Protect the executor's balances and allowances, not the signing wallet's.
...
Syntax (check get_docs before using anything you are not sure of):
  assert <target>::!{viewFn(argTypes)(returnType) <args>} <op> <expected> "revert msg"   # a live read: inside an assertion EVERY call is ::! with its ABI inline; a plain :: call runs at build time and is an error here
...
<op> is == != > >= < <= or ~= with --delta for approximate values. Both sides may be live: assert $a::!{x()(uint256)} > $b::!{y()(uint256)} compares on-chain at execution time. int256 returns compare signed (negative expected values work); string/bytes/bytes32 support == and !=.

On-chain composition: helpers with a trailing ! evaluate ON-CHAIN at assertion time (inside assert only), composed via the core's read primitive splicing live values into the Operations and Collections contracts. Spaces around every operator are mandatory; top-level infix is invalid, wrap it:
  @calc!(<expr>)   on-chain checked integer arithmetic: + - * // % ^ (integer division is //, never /) and parentheses over live ::! calls, ! helpers and constants, e.g. @calc!(@balance!(ETH @me) + @token(WETH)::!{balanceOf(address)(uint256) @me})
  @bool!(<expr>)  on-chain comparisons and logic: == != < <= > >= and or xor not, e.g. @bool!(($gov::!{quorum()(uint256)} > 0) or (not $gov::!{paused()(bool)}))
  ...
  @str.split!(<call> "<delim>" i) string segment: i may be negative (-1 = last, resolved live on-chain), so "name ends with LP" is @str.split!($t::!{name()(string)} " " -1) == "LP". String results support == and != anywhere (nested comparisons compile to on-chain keccak); other operators are invalid on strings.
  ...
  def @name! "<signature>" <body>  defines an ON-CHAIN helper, and is the ONLY way to write the lambda the array faces apply. NEVER write an inline lambda: @map!($v::!{caps()(uint256[])} @calc!(* 2)) does not compile. Name it first, then apply it by name with no arguments, the face supplies them:
    def @dbl! "$x: number -> number" @calc!($x * 2)
    assert @sum!(@map!($v::!{caps()(uint256[])} @dbl!)) > 100
  @map!(<call> @def!) @filter!(<call> @pred!) @all!(<call> @pred!) @any!(<call> @pred!) @find!(<call> @pred!)  one-parameter definitions; predicates return bool. @reduce!(<call> @def! <init>) takes TWO parameters, accumulator first, def @subFrom! "$acc: number $e: number -> number" @calc!($acc - $e), or a bare commutative name (add mul min max bitAnd bitOr bitXor). @sort!(<call> asc|desc) takes a DIRECTION, never a comparator; signed elements sort by value.
  A def must be fully typed (its body compiles, so types cannot be inferred), cannot be called off-chain, and is inlined where used, so naming a parameter twice is meaningful: def @sq! "$x: number -> number" @calc!($x * $x) squares in one call. Recursion is refused; @name and @name! are independent bindings.
  ...
  <head>::!{sig(argTypes)(retTypes) args}  the head of a ::! read is any expression: an address, a ::! chain, an ! helper or a computed word resolved to an address on-chain, e.g. @bytes!($reg::!{packedPool()(uint256)} ">>" 96)::!{fee()(uint24)}. With a fixed head and constant arguments it is a direct staticcall; a live head or live argument constructs the call at assertion time through the core's read. The ! trails the :: so it never sits against the head, where it would be indistinguishable from an ! helper face.
...
- 1:1 wrap/deposit/withdraw (e.g. deposit 0.001 ETH into WETH): exactly 1 post-assertion: the executor's balance of the received asset grew by the amount (balanceOf(@sender) >= @num($before + 0.001e18)). Nothing else.
...
export interface BuilderChatHostOptions {
  ...
  /** The configured tag (chain, executor, transports), through a live
   *  proxy (see `createLiveTag`) so the tool set built here stays stable
   *  while the configuration changes. */
  tag: EvmlTag;
}

/** The builder's chat agent: `@evmcrispr/ai`'s headless hook bound to the
 *  page's script state and the configured tag. */
export function useBuilderChatAgent({ scriptState, tag }: BuilderChatHostOptions) {
  const tools = useMemo(() => {
    const host: ScriptToolsHost = {
      ...
      simulate: (script, { from, blockNumber }) =>
        tag.script(script).simulate({
          from: (from as Address | undefined) ?? tag.config.account,
          blockNumber,
        }),
    };
    return {
      ...
      ...createDocTools(),
      ...createContractTools(() => tag.config.chainId),
      ...createWebTools(),
    };
  }, [scriptState, tag]);
