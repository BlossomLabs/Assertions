...
```evml
assert @num!(@balance!(ETH $addr1) + $weth::balanceOf($addr1)) > 0
```
...
```evml
assert $token::balanceOf($a) >= @num!(5 * 10 ^ $token::decimals())
```
