...
// canonical Assertions/Operations addresses. Use the website's Deployments
...
export default buildModule("AssertionsModule", (m) => {
  ...
  const operators = m.contract("Operations");

  const collectionOperations = m.contract("Collections");

  return { assertions, operators, collectionOperations };
});
