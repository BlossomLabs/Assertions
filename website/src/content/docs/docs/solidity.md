...
Constraints compare the value's **first 32-byte word**, unsigned — which covers `uint256`, `address`, `bool` and `bytes32` returns directly. Signed comparisons, `!=`, and multi-value selections route through the [Combinators](/docs/combinators) (a comparison opcode returns 0/1, judged `EQ 1`).
...
// Assert new implementation has expected code, via the env(CodeHash)
// combinator as the fetched value (EXTCODEHASH semantics)
assertions.assertParam(
    callParam(
        address(combinators),
        abi.encodeCall(Combinators.env, (Combinators.EnvOp.CodeHash, uint256(uint160(newImplementation)))),
...
Balance reads have a dedicated fetcher; block and chain values come from the `env` combinator:
...
assertions.assertParam(
    callParam(address(combinators), abi.encodeCall(Combinators.env, (Combinators.EnvOp.ChainId, 0)), eq(bytes32(uint256(1))))
);

// Assert block timestamp is past the unlock time
assertions.assertParam(
    callParam(address(combinators), abi.encodeCall(Combinators.env, (Combinators.EnvOp.Timestamp, 0)), gte(unlockTime + 1))
);
```

## Multi-value returns and deep reads

Selections out of tuples, arrays and structs are computed by the Combinators — `pick` for a raw word, [`nav`](/docs/combinators/reads) for typed navigation — and judged as ordinary parameters:
...
assertions.assertParam(
    callParam(
        address(combinators),
        abi.encodeCall(Combinators.pick, (
...
- **Constraints are unsigned word comparisons.** `GTE`/`LTE`/`IN` compare the first 32-byte word as a `uint256`. For `int256` returns use the Combinators' signed comparison opcodes (`SGt`, `SLe`, …) judged `EQ 1`, and `SAbsDiff` for signed tolerance.
- **EIP-7702 delegated EOAs carry code.** A delegated EOA has a 23-byte delegation designator as its code, so a "has no code" check (`env(CodeHash)` equal to `bytes32(0)` or `keccak256("")`) is not a strict "is an EOA" check on chains with EIP-7702.
- **`block.number` semantics differ across chains.** On OP-stack and most L2s, `env(BlockNumber)` sees the L2 block number (on Arbitrum, `block.number` returns the approximate L1 block). Block times also vary per chain, so avoid porting block-number thresholds between networks.
- **Calls to code-less addresses revert with `CallFailed`.** A `staticcall` to an address without code would otherwise "succeed" with empty returndata; the fetcher detects this and reverts descriptively.
