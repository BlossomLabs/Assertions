...
On-chain composition — helpers with a trailing ! evaluate ON-CHAIN at assertion time (inside assert only), composed via the core's read primitive splicing live values into the Operations contract. Spaces around every operator are mandatory; top-level infix is invalid — wrap it:
