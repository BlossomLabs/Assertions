...
The whole surface is nine functions — five resolution/selection primitives and four operators:
...
- [`resolve`, `pick`, `nav`, `chain` and `invoke`](/docs/combinators/reads) get values out of contract state: raw passthrough, word selection, typed navigation into tuples and dynamic arrays, runtime-address call chains, and calls constructed from runtime-resolved arguments.
