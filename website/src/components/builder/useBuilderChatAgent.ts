...
The script you edit is the inner action block only — the site wraps it into the final batch/proposal, so never add batch, safe:propose, governor:propose or aragonosx wrappers yourself. Put "load assertions" on its own line at the top of the block (the site hoists load lines when wrapping), and add the owning module's load line for its helpers: "load lang" for the array/string faces (@len!, @at!, @str.*!, @bytes.len!, ...), "load receipts" for the block/tx context reads (@block.*!, @tx.from!, @tx.gasprice!, @tx.blobhash!, @receipts:tx*), "load math" for @min!/@max!/@absdiff!/@sqrt!. Assertions execute atomically with the surrounding calls: a failed assertion reverts the entire batch.
...
Syntax (check get_docs before using anything you are not sure of):
  ...
  assertions:assert <t>::{owners()(address[],address)}[[_ $]] == <addr>              # a NESTED lens navigates the final return: each nesting level is one step into an array (element by position, live-bounds-checked on-chain) or a struct/tuple (field by position). Any depth: {matrix()(address[][])}[[_ [$]]] = [1][0]; {proposals()((address,uint256,bool)[])}[[_ [_ _ $]]] = proposals[1].executed. A ... rest marker anchors the slots after it from the end: [... $] = last return value, [[... $]] = last array element (resolved live on-chain). Works nested in @bool!/@num!, and @len!/@str.split!/@str.includes!/@str.charset!/@hash!/@bytes.len! accept a lensed call selecting a nested string/array, e.g. @len!($t::{matrix()(address[][])}[[$]])
...
On-chain composition — helpers with a trailing ! evaluate ON-CHAIN at assertion time (inside assertions:assert only), composed via the core's read primitive splicing live values into the Operators contract. Spaces around every operator are mandatory; top-level infix is invalid — wrap it:
  ...
  @str.includes!(<call> "part") string-contains and @str.charset!(<call> "a-z0-9-") only-these-characters (ranges + literals; byte-level ASCII) — both bool-valued, usable bare, with == true/false, or nested in @bool!. More lang faces exist: @str.slice!/@str.at!/@str.concat!/@str.replace!/@str.lower!/@str.upper!/@str.join!, bytes @bytes.at!/@bytes.slice!/@bytes.concat!, array @at!/@slice!/@includes!/@all!/@any!/@map!/@filter!/@find!/@reduce!/@sum!/@sort!/@unique!/@reverse!/@zip!/@unzip!/@enumerate!/@flat!/@concat!, records @keys!/@values!/@lookup! (a record is the zipped key/value word-pair payload @zip!/@enumerate! produce; string keys travel as keccak digests; @find!/@lookup! revert when nothing matches).
  @ok!(<call>)  whether a live call resolves without reverting (true/false), checked on-chain at assertion time.
  <head>!::{sig(argTypes)(retTypes) args}  constructs a whole call at assertion time via the core's read: the head is any expression resolving to an address on-chain (a :: chain, an ! helper, a computed word), inline ABI mandatory, e.g. @bytes!($reg::packedPool() ">>" 96)!::{fee()(uint24)}.
  @bytes!(a "&" b)  on-chain bitwise ops over 32-byte words — "&" "|" "^" "<<" ">>" (operator quoted); single-arg @bytes!(x) casts the raw word (the explicit bool→number bridge). @not!(x) negates on-chain: logical not on bools (stays bool), bitwise complement on numbers/bytes32.
  @chainid!  the live chain id, composable (unlike assert-chainid's constant comparison)
  @codehash!(<addr-or-call>)  live EXTCODEHASH — the argument may be a ::-call resolving to an address, so a proxy upgrade check is @codehash!($proxy::{implementation()(address)}) == <bytes32>. Distinct from @codehash(<addr>) (no !), which snapshots the hash at composition time; both use EXTCODEHASH semantics (nonexistent account = 0x0).
