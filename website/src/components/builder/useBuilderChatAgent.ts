...
On-chain composition: helpers with a trailing ! evaluate ON-CHAIN at assertion time (inside assert only), composed via the core's read primitive splicing live values into the Operations and Collections contracts. Spaces around every operator are mandatory; top-level infix is invalid, wrap it:
  @calc!(<expr>)   on-chain checked integer arithmetic: + - * // % ^ (integer division is //, never /) and parentheses over live ::-calls, ! helpers and constants, e.g. @calc!(@balance!(ETH @me) + @token(WETH)::balanceOf(@me))
  ...
  def @name! "<signature>" <body>  defines an ON-CHAIN helper, and is the ONLY way to write the lambda the array faces apply. NEVER write an inline lambda: @map!($v::caps() @calc!(* 2)) does not compile. Name it first, then apply it by name with no arguments, the face supplies them:
