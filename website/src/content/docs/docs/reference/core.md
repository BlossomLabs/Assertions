...
The judge lives at `0xa55E47F37088b6D0212BdfD56b175ec08744DB19`. Every judge function has an overloaded version accepting a custom `string` message as the last parameter, echoed inside `ConstraintFailed` on failure. The composition functions live on the separate [Combinators contract](/docs/combinators).
...
These live at the Combinators address (`0xA55Ec0935FB5aaf95CAC1F48DD822005d91b64b9`), not on the judge. Every operand is an `InputParam` (with its own inline constraints), so expressions nest recursively. See [Combinators](/docs/combinators) for usage.
...
| `invoke` | Construct a staticcall at judge time: resolve the target and concatenate the selector with each argument segment's full resolved bytes (ERC-8211 CALL_DATA routing), then return the call's raw returndata — any external view function becomes callable with computed arguments |
