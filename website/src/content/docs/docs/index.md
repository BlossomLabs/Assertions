...
- **ERC-8211 native**: assertions are standard Smart Batching predicate entries; canonical predicate constraints follow Biconomy's reference semantics, subject to the view restrictions and canonical payload lengths documented below
- **Inline constraints**: unsigned/signed comparisons, inclusive ranges, `OR` and `SKIP` validate successive words of a fetched value ([how constraints judge](/docs/core/reads#constraints))
